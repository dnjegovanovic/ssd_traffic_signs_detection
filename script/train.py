...
from network.losses import createLosses
import math

parser = argparse.ArgumentParser()
parser.add_argument('--data-dir', default=r'D:\FAX\MASTER\LISADFG')
...
parser.add_argument('--checkpoint-dir', default='checkpointsNewData')
...
def train_net():
    ...
    batch_generator, val_generator, info = createTraffBatchGenerator(args.data_dir,
                                                               args.img_dir,
                                                               default_boxes,
                                                               config['image_size'],
                                                               args.batch_size,
    ...
    criterion = createLosses(args.neg_ratio, NUM_CLASSES)
    steps_per_epoch = math.ceil(info['length'] / args.batch_size)
    ...
    train_log_dir = 'logs/trainAugND'
    val_log_dir = 'logs/valAugNd'
    ...
    for epoch in range(args.num_epochs):
        ...
        if (epoch + 1) % 5 == 0:
            ssd.save_weights(
                os.path.join(args.checkpoint_dir, 'ssd_epoch_{}.h5'.format(latestEpoch+epoch + 1)))